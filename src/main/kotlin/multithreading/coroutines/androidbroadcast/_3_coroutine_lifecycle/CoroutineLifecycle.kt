 ...
 * Любая корутина, запускаемая в скоупе, будет храниться ссылкой в нём через отношение "родитель-ребёнок" у Job. //TODO Посмотреть, есть ли дефолтный Job у скоупа, и можно ли через него получить доступ к дочерним джобам.
 * 3. Я тебя буду ждать.
 ...
 * Часто полезно использовать SupervisorJob - тогда ошибки из любой дочерней корутины не приведут к отмене всех корутин в скоупе. //TODO: Сравнить SupervisorJob и Job для скоупа при отмене дочерней корутины.
 * Не обязательно задавать Dispatcher для CoroutineScope, тк будет использован вариант по умолчанию - Dispatchers.Default.
 * Но лучше задавать, что всё контролировать.
 ...
 * Для создания CoroutineScope функция coroutineScope возьмёт контекст из родителя и добавит к нему Job, который будет связан с внешним скоупом. //TODO: Проверить связь "родитель-ребёнок".
 * CoroutineScope, созданные функцией coroutineScope будет работать по следующим правилам: //TODO: Проверить как работают правила.
 ...
 *      }
 * Этот скоуп формируется по следующим правилам: //TODO: Посмотреть как меняется контекст в StructuredConcurrency
