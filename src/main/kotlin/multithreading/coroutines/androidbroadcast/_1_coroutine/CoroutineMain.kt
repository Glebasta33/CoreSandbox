 ...
 *
 * Корутины позволяют выполнять асинхронные опирации, которые могут быть преостановленны, а позже продолжить своё выполнение.
 * Корутины сравнивают с потоками, но это не верно. Потоки применяются для организации многозадачности, корутины же предоставляют лишь асинхрнонные операции, но не выполняются параллельно.
 ...
 *
 * ## Ассинхронность VS Параллельность.
 * Ассинхронность: события происходят независимо от главного потока. Способ взаимодействия с главным потоком не является последовательным, а происходит через коллбеки.
 ...
 *
 * Корутины позволяют писать ассинхронный код (с коллбеками) в синхронном стиле (без использования коллбеков):
 *
 *      //Асинхронный стиль
 *      remoteCall { data ->
 *          storeData(data) {
