 ...
 * async и produce - требуют обработки исключений от того, кто вызовет код.
 *
 ...
 *     }
 *
 *     (Но у меня упал всё-равно поток)
 *     //TODO: Разбораться почему исключение не захватывается в Deferred?
 *
 ...
 * Можно подписаться на уведомление о выполении корутины. Но этот способ не перехватит исключение:
 *
 ...
 * Поэтому при обработке через try-catch в корутинах нужно пробрасывать CancellationException далее.
 *
